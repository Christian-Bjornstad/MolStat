...
from molstat.lvms.batch_form import BatchFormError
...
from molstat.lvms.cdp import CdpNavigationError, CdpTimeout
        ...
        class Navigator:
            def reach(
                ...
                *,
                ...
                del page, actions
                if harness.failure_stage == "navigation":
                    raise BatchNavigationError("synthetic navigation failure")
        ...
        class Form:
            ...
            def populate(self, contract: DefinedReportsPage, job: ReportJob) -> None:
                ...
                if harness.failure_stage == "populate" and job.job_key == harness.failed_job:
                    raise BatchFormError("synthetic populate failure")
...
class BatchRunnerTests(unittest.TestCase):
    ...
    def test_reports_safe_form_stage_when_report_population_fails(self) -> None:
        harness = BatchHarness("populate")

        result, output = self.run_harness(harness)

        self.assertEqual(result, 2)
        self.assertIn("rapportskjema", output.lower())
        self.assertIn("report_2_fill", output)

    def test_reports_safe_navigation_stage_when_defined_reports_cannot_be_reached(self) -> None:
        harness = BatchHarness("navigation")

        result, output = self.run_harness(harness)

        self.assertEqual(result, 2)
        self.assertIn("Definerte rapporter", output)
        self.assertIn("(defined_reports)", output)
