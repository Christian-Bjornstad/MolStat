...
def aggregate_analyses(
    ...
    generated_at: datetime,
) -> tuple[AnalysisDashboard, ...]:
    dashboards: list[AnalysisDashboard] = []
    for analysis in config.analyses:
        if not analysis.enabled:
            continue
        selected = [item for item in samples if item.analysis_code == analysis.code]
        ready_samples = [
            item for item in selected if item.stage is WorkflowStage.READY
        ]
        awaiting = sum(
            item.stage is WorkflowStage.AWAITING_APPROVAL for item in selected
        )
        in_transit = sum(
            item.stage is WorkflowStage.IN_TRANSIT for item in selected
        )
...
def build_dashboard_snapshot(
    ...
    *,
    ...
    aggregates = aggregate_analyses(config, samples, generated_at)
    dashboards = [
        (index, dashboard)
        for index, dashboard in enumerate(aggregates)
        if dashboard.severity is not Severity.EMPTY
    ]
    empty_analysis_count = sum(
        dashboard.severity is Severity.EMPTY for dashboard in aggregates
    )
    severity_rank = {
        Severity.OVERDUE: 0,
        Severity.WARNING: 1,
