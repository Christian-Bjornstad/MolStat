...
# Compatibility model kept until system routing is migrated to DEFAULT_UNITS.
@dataclass(frozen=True, slots=True)
class DataModule:
    key: str
    display_name: str
...
DEFAULT_MODULES = ModuleRegistry(
    (
        DataModule(
            ...
            display_name="Hemato",
            job_kind="statistics",
            schedule_key=_HEMATO_STATISTICS.schedule_key,
            processor_profile=_HEMATO_STATISTICS.processor_profile,
            sharepoint_folder=_HEMATO_STATISTICS.sharepoint_folder,
            publication_files=_HEMATO_STATISTICS.publication_files,
            status_fields=_HEMATO_STATISTICS.status_fields,
        ),
        DataModule(
            key="solide",
            display_name="Solide",
            job_kind="statistics",
            schedule_key=_SOLIDE_STATISTICS.schedule_key,
            processor_profile=_SOLIDE_STATISTICS.processor_profile,
            sharepoint_folder=_SOLIDE_STATISTICS.sharepoint_folder,
            publication_files=_SOLIDE_STATISTICS.publication_files,
            status_fields=_SOLIDE_STATISTICS.status_fields,
        ),
        DataModule(
            key="proveflyt",
            display_name="Prøveflyt",
            job_kind="backlog",
            schedule_key=_HEMATO_BACKLOG.schedule_key,
            processor_profile=_HEMATO_BACKLOG.processor_profile,
            sharepoint_folder=_HEMATO_BACKLOG.sharepoint_folder,
            publication_files=(
                ("restansehistorikk.csv", frozenset(BACKLOG_PUBLIC_COLUMNS)),
            ),
            status_fields=_HEMATO_BACKLOG.status_fields,
        ),
    )
)
