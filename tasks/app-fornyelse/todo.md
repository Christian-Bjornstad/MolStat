...
Hovedplan: [plan.md](plan.md). Alle oppgaver under er planlagt, ikke implementert.
...
- [ ] Strukturert feil viser trinn, kode, kjørings-ID og forsøk uten sensitive data.
- [ ] Syntetiske navigasjons-, nedlastings- og oppryddingsfeil beholder riktig årsak.
- [ ] Verifiser med `python -m pytest tests/test_fetching.py tests/lvms/test_batch_runner.py tests/test_services.py -q` og nye feiltester.
...
- [ ] Lag reproduksjon eller dokumenter miljøavhengig begrensning før retting; skill tilgang, lås, sti og skjema.
- [ ] Gjentatt åpning/restart bevarer identiteter og historikk; forbindelser lukkes deterministisk.
...
- [ ] Ingen automatisk opprettelse av erstatningsdatabase ved åpnefeil.
...
- [ ] Definer referanser, aktivering, profiler, CSV-kontrakt og endrings-/historikkregler.
- [ ] Test at malene er gyldige og at dagens oppsett kan representeres uten informasjonstap.
...
- [ ] Feil i JSON, duplikate nøkler, typer, referanser og lister gir presis felt-/linjefeil.
...
- [ ] Test Qt-flyten og `python -m pytest tests/test_config.py tests/test_services.py tests/ui/test_control_center.py -q`.
...
- [ ] Les/valider før kjøring; samme snapshot brukes til uthenting og behandling.
...
- [ ] Fjern fast Hemato-ruting der samme støttede restanseprofil kan brukes for flere enheter.
...
- [ ] Feil fil blokkerer før LVMS, og eksisterende Power BI-output har samme kontrakt.
...
- [ ] Kartlegg eksisterende rot og behold `data/molstat.sqlite3` og `Prøvesøk.xlsx`.
...
- [ ] Råarkiv, midlertidig restanse og prosesserte kandidater følger mappekontrakten og kjørings-ID.
- [ ] Låst SharePoint-fil kan prøves igjen fra ferdig kandidat; siste gyldige output beholdes.
- [ ] Verifiser med `python -m pytest tests/test_archive.py tests/test_publisher.py tests/test_privacy_boundary.py tests/test_end_to_end.py -q`.
...
- [ ] Uavklart rapportstatus undersøkes før ny innsending; databaseimport og historikk dupliseres ikke.
...
- [ ] Appen skiller gammel lesekopi/låst fil/feilet eksport fra vellykket databaseimport og kan regenerere uten LVMS.
...
- [ ] Kjør full `python -m pytest -q`, `python -m compileall -q src` og `git diff --check`; dokumenter eventuelle eksisterende feil uten å skjule dem.
...
- [ ] Dokumenter filredigering, nye analyser, tilbakeføring, feilkoder og retensjon; knytt evidens til gamle åpne kontrollpunkter før de lukkes.
